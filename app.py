...
try:
    ...
    st.error(f"Failed to import directreservation module: {e}")
    st.stop()

try:
    from online_reservation import show_online_reservations, load_online_reservations_from_supabase
except Exception as e:
    st.error(f"Failed to import online_reservation module: {e}")
    show_online_reservations = lambda: st.write("Online Reservations not available.")
    load_online_reservations_from_supabase = lambda: []
...
try:
    ...
    show_edit_online_reservations = lambda x: st.write("Edit Online Reservations not available.")
    edit_online_available = False

try:
    from inventory import show_daily_status
except Exception as e:
    st.error(f"Failed to import inventory module: {e}")
    show_daily_status = lambda: st.write("Daily Status not available.")

try:
    from dms import show_dms
except Exception as e:
    st.error(f"Failed to import dms module: {e}")
    show_dms = lambda: st.write("Daily Management Status not available.")

try:
    from monthlyconsolidation import show_monthly_consolidation
except Exception as e:
    st.error(f"Failed to import monthlyconsolidation module: {e}")
    show_monthly_consolidation = lambda: st.write("Monthly Consolidation not available.")

try:
    from log import show_log_report, log_activity
except Exception as e:
    st.error(f"Failed to import log module: {e}")
    show_log_report = lambda x: st.write("Log Report not available.")
    log_activity = lambda x, y, z: None
...
# Cache data fetching
@st.cache_data
def cached_load_reservations_from_supabase():
    return load_reservations_from_supabase()

@st.cache_data
def cached_load_online_reservations_from_supabase():
    return load_online_reservations_from_supabase()

# Store property room map in session state
st.session_state.property_room_map = {
    ...
    if not st.session_state.authenticated:
        ...
        if st.button("🔑 Login"):
            if username == "Admin" and password == "Admin2024":
                ...
                try:
                    users = st.session_state.supabase.table("users").select("*").eq("username", username).eq("password_hash", password).execute().data
                    if users and len(users) == 1:
                        ...
                        if st.session_state.role == "Admin":
                            ...
                            valid_screens.append("Analytics")
            ...
            if st.session_state.authenticated:
                ...
                try:
                    st.session_state.reservations = cached_load_reservations_from_supabase()
                    st.session_state.online_reservations = cached_load_online_reservations_from_supabase()
...
def show_user_management():
    ...
    with st.form("create_user_form"):
        ...
        all_properties = list(st.session_state.property_room_map.keys())
        ...
        if st.form_submit_button("Create User"):
            existing = st.session_state.supabase.table("users").select("*").eq("username", new_username).execute().data
            if existing:
                ...
                try:
                    st.session_state.supabase.table("users").insert(new_user).execute()
                    log_activity(st.session_state.supabase, st.session_state.username, f"Created user {new_username}")
    ...
    if modify_username:
        ...
        with st.form("modify_user_form"):
            ...
            if st.form_submit_button("Update User"):
                ...
                try:
                    st.session_state.supabase.table("users").update(updated_user).eq("username", modify_username).execute()
                    log_activity(st.session_state.supabase, st.session_state.username, f"Modified user {modify_username}")
    ...
    if delete_username and st.button("Delete User"):
        try:
            st.session_state.supabase.table("users").delete().eq("username", delete_username).execute()
            log_activity(st.session_state.supabase, st.session_state.username, f"Deleted user {delete_username}")
...
def main():
    ...
    if st.sidebar.button("🔄 Refresh All Data"):
        ...
        try:
            st.session_state.reservations = cached_load_reservations_from_supabase()
            st.session_state.online_reservations = cached_load_online_reservations_from_supabase()
            log_activity(st.session_state.supabase, st.session_state.username, "Refreshed all data")
    ...
    if page == "Direct Reservations":
        ...
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed Direct Reservations")
    elif page == "View Reservations":
        show_reservations()
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed View Reservations")
    elif page == "Edit Reservations":
        show_edit_reservations()
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed Edit Reservations")
    elif page == "Online Reservations":
        show_online_reservations()
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed Online Reservations")
        ...
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed Edit Online Reservations")
    elif page == "Daily Status":
        show_daily_status()
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed Daily Status")
    elif page == "Daily Management Status":
        show_dms()
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed Daily Management Status")
    elif page == "Analytics" and st.session_state.role == "Management":
        show_analytics()
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed Analytics")
    elif page == "Monthly Consolidation":
        show_monthly_consolidation()
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed Monthly Consolidation")
    elif page == "User Management" and st.session_state.role == "Admin":
        show_user_management()
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed User Management")
    elif page == "Log Report" and st.session_state.role == "Admin":
        show_log_report(st.session_state.supabase)
        log_activity(st.session_state.supabase, st.session_state.username, "Accessed Log Report")

    if st.session_state.authenticated:
        st.sidebar.write(f"Logged in as: {st.session_state.username}")
    if st.sidebar.button("Log Out"):
        log_activity(st.session_state.supabase, st.session_state.username, "Logged out")
        st.cache_data.clear()
