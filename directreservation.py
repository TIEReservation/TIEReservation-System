...
def show_new_reservation_form():
    ...
    try:
        ...
        with row4_col4:
            if room_type == "Others":
                room_no = st.text_input("Room No", value="", placeholder="Enter custom room number", key=f"{form_key}_room_no", help="Enter a custom room number for 'Others' room type.")
                ...
                room_no = st.text_input("Room No", value="", placeholder="Enter room number", key=f"{form_key}_room_no", help="Enter or edit the room number. You can type any custom value or use suggestions below.")
        ...
        if mob == "Online":
            ...
            with row10_col1:
                ...
                online_source = st.selectbox("Online Source", online_source_options, key=f"{form_key}_online_source")
                ...
                if not booking_id:
                    ...
                    if is_duplicate:
                        ...
                        if save_reservation_to_supabase(new_reservation):
                            st.session_state.reservations.insert(0, new_reservation)
...
def show_edit_reservations():
    ...
    try:
        ...
        st.subheader("🔍 Search Reservation to Edit")
        col_search1, col_search2 = st.columns([3, 1])
        with col_search1:
            direct_booking_id = st.text_input("Enter Booking ID to Search", placeholder="Enter Booking ID (e.g., TIE20251220001)", key="search_input", help="Enter exact Booking ID to directly edit a reservation")
        with col_search2:
        ...
        if st.session_state.get('edit_mode', False) and st.session_state.get('edit_index') is not None:
            show_edit_form(st.session_state.edit_index)
        else:
            st.info("👆 Please search for a Booking ID above to load and edit a reservation.")
    ...
    try:
        ...
        with row4_col4:
            if room_type == "Others":
                ...
                room_no = st.text_input("Room No", value=initial_value, placeholder="Enter custom room number", key=f"{form_key}_room_no", help="Enter a custom room number for 'Others' room type.")
                if not room_no.strip():
                    st.warning("⚠️ Please enter a valid Room No for 'Others' room type.")
            else:
                room_numbers = property_room_map[property_name].get(room_type, [])
                room_no = st.text_input("Room No", value=fetched_room_no, placeholder="Enter room number", key=f"{form_key}_room_no", help="Enter or edit the room number. You can type any custom value or use suggestions below.")
                suggestion_list = [r for r in room_numbers if r.strip()]
                if suggestion_list:
                    st.caption(f"💡 **Quick suggestions:** {', '.join(suggestion_list)}")
        ...
        with row9_col2:
            # Submitted By shows original submitter (read-only)
        ...
        with col_btn3:
            if st.button("🔄 Load Another Reservation", use_container_width=True):
                st.session_state.edit_mode = False
                st.session_state.edit_index = None
                st.rerun()
    except Exception as e:
...
if __name__ == "__main__":
    ...
    with st.sidebar:
        ...
        if st.session_state.username:
            ...
            st.info(f"Current Date: December 20, 2025")
        else:
